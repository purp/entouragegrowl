(*
	...
	To be executed by a rule in Entourage (e.g. all messages as your last rule, or 'from' 'is in Address Book'). Assumes that any message passed to it should be growled as sticky.
*)
			...
			repeat with thisMessage in the current messages
  			...
  			# Show the notification
				tell application "GrowlHelperApp"
					notify with name "New Email" title ("You have " & mailDescriptor & " email") description ("From " & mailSender & " about " & mailSubject) application name "Microsoft Entourage" priority noticePriority sticky yes
				end tell
		  end repeat
		end tell
	end if
